...
animals_list = [Animal("lion"), Animal("mouse")]
...
animals_list = [Animal("lion"), Animal("mouse"), Animal("snake")]
...
"""
Animal now has a virtual method make_sound. We have each animal extend the Animal 
class and implement the virtual make_sound method.

Every animal adds its own implementation on how it makes a sound in the make_sound. 
The animal_sound iterates through the array of animal 
and just calls its make_sound method.
...
To make it follow the OCP principle, we will add a new class 
that will extend the Discount. 
